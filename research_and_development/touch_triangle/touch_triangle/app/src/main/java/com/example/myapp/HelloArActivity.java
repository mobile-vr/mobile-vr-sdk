...
public class HelloArActivity extends AppCompatActivity implements SampleRender.Renderer {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    // Initialize the variables of your game here.

    // Point to keep the intersection information
    point = new IntersectionPoint();
    // Timer to change the color after 1s
    timer = System.currentTimeMillis();
    // Counter to choose which color to use
    counter = 0;
    // initial color
    color = new float[]{0.0f, 1.0f, 0.0f, 0.8f}; // GREEN
   ...
   */
    ...
    public void onSurfaceCreated (SampleRender render){
      ...
      // Triangle
      COORDS_PER_VERTEX = 3;
      float[] coloredTriangleCoords = {   // in counterclockwise order:
              0.0f, 0.5f, -2.0f,  // top
              -0.5f, -0.2f, -2.0f,  // bottom left
              0.5f, -0.2f, -2.0f,  // bottom right
      };

      // Interfaces to interact with
      float[][] coloredTriangleCoordsIFs = {
              coloredTriangleCoords
      };

      // indexes
      int[] coloredTriangleIndex = {   // in counterclockwise order:
              0,  1, 2
      };
      ...
      triangleObject = new VirtualObject(
              ...
              COORDS_PER_VERTEX,
              coloredTriangleCoords,
              coloredTriangleIndex,
              vertexShaderFileName,
              fragmentShaderFileName,
              coloredTriangleCoordsIFs,
              mode);
     ...
     */
      ...
      // Check if user's finger and thumb are touching above the triangle's interface
      checkTouchingIF(
              cameraPose,
              landmarkList,
              triangleObject.IFs[0],
              point,
              fingerQuaternion);

      // Change the color if finger and thumb are touching via the boolean point.action
      if (point!= null && point.action && System.currentTimeMillis() - timer > 500.0f) {
        counter += 1;
        timer = System.currentTimeMillis();
        if (counter % 3 == 0) {
          color = new float[]{1.0f, 0.0f, 0.0f, 0.8f}; // RED
        }else if (counter % 3 == 1) {
          color = new float[]{0.0f, 1.0f, 0.0f, 0.8f}; // GREEN
        }else if (counter % 3 == 2) {
          color = new float[]{0.0f, 0.0f, 1.0f, 0.8f}; // BLUE
        }
      }
      ...
      triangleObject.shader.setVec4("vColor", color);
