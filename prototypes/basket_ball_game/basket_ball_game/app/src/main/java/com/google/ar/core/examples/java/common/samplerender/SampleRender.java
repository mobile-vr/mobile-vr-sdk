...
public class SampleRender {
   ...
   * of the frame, and (u, v) a vector.
   *
   * x0, y0, u, v are factors to position the viewport to adapt it
   * to the user's cardboard. It is a percentage (from 0 to 1).
   * y0 + v = 0.5 to center the line between the 2 views at the screen center in landscape view.
   ...
   * @param u: x vector component
   * @param v: y vector component
   */
  private void useFramebuffer(Framebuffer framebuffer, int side, float x0, float y0, float u, float v) {
    ...
    if (framebuffer == null) {
      ...
      if (side == 0) {
        //GLES30.glViewport(0, 0, viewportWidth, (int) Math.round(viewportHeight/2));

        // This seems perfect but the virtual scene is upon the other
        //GLES30.glViewport(0, (int) Math.round(viewportHeight*0.08), viewportWidth, (int) Math.round(viewportHeight*1.08));
        GLES30.glViewport(
        ...
        //GLES30.glViewport(0, (int) Math.round(viewportHeight*0.08), viewportWidth, (int) Math.round(viewportHeight*0.8));

        // TOP

      } else if (side == 1) {
        //GLES30.glViewport(0, (int) Math.round(viewportHeight*0.5), viewportWidth, viewportHeight);
        GLES30.glViewport(
                (int) Math.round(viewportWidth * x0),
      ...
      if (side == 0) {
        GLES30.glBindFramebuffer(GLES30.GL_FRAMEBUFFER, framebufferId);
        GLError.maybeThrowGLException("Failed to bind framebuffer", "glBindFramebuffer");
        //GLES30.glViewport(0, (int) Math.round(viewportHeight*0.92), viewportWidth, viewportHeight);
        //GLES30.glViewport(0, (int) Math.round(viewportHeight*0), viewportWidth, (int) Math.round(viewportHeight*0.5)); // ici,
        // seule la partie haute est affichée sans carré vert, alors que quand on commente le GLES30.glViewport du dessous
        // on a et l'image derrière et le carré vert, c'est fort étrange.
        ...
        GLError.maybeThrowGLException("Failed to bind framebuffer", "glBindFramebuffer");
        //GLES30.glViewport(0, 0, viewportWidth, viewportHeight);
        //GLES30.glViewport(0, (int) Math.round(viewportHeight*0.5), viewportWidth, (int) Math.round(viewportHeight*0.92));
        //GLES30.glViewport(0, (int) Math.round(viewportHeight*0.5), viewportWidth, (int) Math.round(viewportHeight*0.5));
        ...
        GLError.maybeThrowGLException("Failed to bind framebuffer", "glBindFramebuffer");
        //GLES30.glViewport(0, 0, viewportWidth, viewportHeight);
        //GLES30.glViewport(0, (int) Math.round(viewportHeight*0.08), viewportWidth, (int) Math.round(viewportHeight*0.92));
        GLES30.glViewport(0, 0, viewportWidth, viewportHeight);
        GLError.maybeThrowGLException("Failed to set viewport dimensions", "glViewport");
      }
    }
